...
G4VUserDetectorConstruction(), 
  ...
  fConstructed(false), fConstructedSDandField(false), fBidimensional(false), fWorldMaterial(0), fDetectorMaterial(0), fMaskMaterial(0),
  fSubMaterial(0),fMuscleMaterial(0),fMySphereMaterial(0),fWaxInsertMaterial(0),
  
  // Some solid declarations:
  fWorldSolid(0), fPixiRadSolid(0), fPixelSolid(0), fMuscleSolid(0), fMySphereSolid(0), fWaxInsertSolid(0), 

  // Some logical declarations:
  fWorldLogical(0), fPixiRadLogical(0), fPixelLogical(0), fM1subLogical(0), fM2subLogical(0), fEnvelopeM2Logical(0), fEnvelopeM1Logical(0), fMuscleLogical(0), fMySphereLogical(0), fWaxInsertLogical(0), 
  // Some physical declarations:
  fWorldPhysical(0), fPixiRadPhysical(0), fPixelPhysical(0), fM1subPhysical(0), fM2subPhysical(0), fEnvelopeM2Physical(0), fEnvelopeM1Physical(0), fMusclePhysical(0), fMySpherePhysical(0), fWaxInsertPhysical(0),
...
{ 
  ...
  densityAorta,densityWax,densityMass;
  ...
  nameFGR,nameADP,nameDBT,nameDBT1,namePQ595,namePQ3070,namePQ5050,nameSilicone,namePMMA,nameMuscle,nameAorta,nameWax,nameMass;
  ...
  ncomponentsMuscle,ncomponentsAorta,ncomponentsWax,ncomponentsMass;
...
//  aAg = 107.86*g/mole;
  ...
  //Mass-M015
  densityMass = 0.9885*g/cm3;
  G4Material* Mass = new G4Material(nameMass="Mass",densityMass,ncomponentsMass=2);
  Mass ->AddMaterial(Adipose, fractionmassDBT1=45*perCent);
  Mass ->AddMaterial(FibroGland, fractionmassDBT1=55*perCent);
  ...
  fMySphereMaterial  = StandardBreast;
  fMaskMaterial      = Vacuum;
  fSubMaterial	     = Vacuum;
  fWaxInsertMaterial = Wax;
  // Add the materials of the objects you create:
...
void PepiDetectorConstruction::DefineVolumes()
{
  ...
  G4ThreeVector positionPixirad = G4ThreeVector(0, 0, fSourcePosZ + fSrcObjDistance + fObjectDetDistance + 2.5*mm);
...
// Phantom Base:
fMuscleSolid = new G4Box("Cube", 10.8*cm/2, 10.15*cm/2, fObjSizeR);
fMuscleLogical = new G4LogicalVolume(fMuscleSolid, fMuscleMaterial, "CubeLV");
G4ThreeVector objectPositionCube = G4ThreeVector(0*mm, 0, fSourcePosZ+fSrcObjDistance-6*mm-fObjSizeR-4*mm);
fMusclePhysical = new G4PVPlacement(0, objectPositionCube, fMuscleLogical, "Cube", fWorldLogical, false, 0, fCheckOverlaps);

// Mass
// My Sphere
fMySphereSolid = new G4Ellipsoid("MySphere",6*mm,6*mm,1*mm,0,0);
fMySphereLogical = new G4LogicalVolume(fMySphereSolid, fMySphereMaterial, "MySphereLV");
G4ThreeVector objectPositionMySphere = G4ThreeVector(0*mm, 0, fSourcePosZ+fSrcObjDistance);
fMySpherePhysical = new G4PVPlacement(0, objectPositionMySphere, fMySphereLogical,"MySphere",fWorldLogical, false, 0, fCheckOverlaps);

// Tissue - Wax
fWaxInsertSolid = new G4Box("MyWax", 8.2*cm/2, 8.15*cm/2, 0.725*cm/2);
fWaxInsertLogical = new G4LogicalVolume(fWaxInsertSolid, fWaxInsertMaterial, "MyWaxLV");
G4ThreeVector objectPositionWax = G4ThreeVector(0*mm, 0, fSourcePosZ+fSrcObjDistance-6*mm );
fWaxInsertPhysical = new G4PVPlacement(0, objectPositionWax, fWaxInsertLogical, "MyWax", fWorldLogical, false, 0, fCheckOverlaps);
...
// Here's a little example of how to create a simple cube and a sphere. Look for any other kind of geometry in the Geant4 Book for Application Developers:
  // https://geant4-userdoc.web.cern.ch/UsersGuides/ForApplicationDeveloper/fo/BookForApplicationDevelopers.pdf, Chapter 4.

/*
fBox = new G4Box("Box", 6.05*cm/2, 6.2*cm/2, fObjSizeR);
fBoxLogical = new G4LogicalVolume(fBox, fBoxMaterial, "BoxLV");
G4ThreeVector objectPositionBox = G4ThreeVector(0*mm, 0, fSourcePosZ + fSrcObjDistance);
fBoxPhysical = new G4PVPlacement(0, objectPositionBox, fBoxLogical, "Box", fWorldLogical, false, 0, fCheckOverlaps);

fSphere = new G4Sphere("Sphere", 0, 750*um/2, 0, 2*pi, 0, pi);
fSphereLogical = new G4LogicalVolume(fMicroSphereSolid1, fMicroSphereMaterial, "Sphere");
G4ThreeVector objectPositionSphere = G4ThreeVector(1, -1*mm, 0);
fSpherePhysical = new G4PVPlacement(0, objectPositionSphere, fSphereLogical, "Sphere", fMuscleLogical, false, 0, fCheckOverlaps);
*/
...
// ================================================================================
//                      VISUALIZATION OF THE CREATED OBJECT
// ================================================================================
  ...
  G4VisAttributes* mysphereVisAtt = new G4VisAttributes(G4Colour(1, 1, 1.0, 1.0));
  mysphereVisAtt->SetForceSolid(true);
  fMySphereLogical->SetVisAttributes(mysphereVisAtt);
...
// Here is a little example to replicate the visualization of an object:

/*
  G4VisAttributes* ObjectVisAtt = new G4VisAttributes(G4Colour(0.6,0.8,1.0,0.5)); // Defines the colour of the object.
  ObjectVisAtt->SetForceSolid(true); // Will the object be seen as a solid?
  fMuscleLogical->SetVisAttributes(ObjectVisAtt); // Links the visualization attributes with the object.
*/
