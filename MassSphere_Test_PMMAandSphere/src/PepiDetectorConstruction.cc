...
void PepiDetectorConstruction::DefineVolumes()
{
  ...
  G4ThreeVector positionPixirad = G4ThreeVector(0, 0, fSourcePosZ + fSrcObjDistance + fObjectDetDistance + fObjSizeR + 2*mm);
...
fMuscleSolid = new G4Box("Cube", 10*cm/2, 10*cm/2, fObjSizeR);
...
// Mass
// My Sphere
fMySphereSolid = new G4Sphere("MySphere",0,0.5*cm,0,2*pi,0,pi);
fMySphereLogical = new G4LogicalVolume(fMySphereSolid, fMySphereMaterial, "MySphereLV");
G4ThreeVector objectPositionMySphere = G4ThreeVector(0,0, 0);
fMySpherePhysical = new G4PVPlacement(0, objectPositionMySphere, fMySphereLogical,"MySphere",fMuscleLogical, false, 0, fCheckOverlaps);
